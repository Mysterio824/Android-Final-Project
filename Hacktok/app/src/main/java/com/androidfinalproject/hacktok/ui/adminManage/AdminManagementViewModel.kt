    ...
    fun onAction(action: AdminManagementAction) {
        when (action) {
            ...
            is AdminManagementAction.DeleteUser -> {
                state = state.copy(
                    users = state.users.filter { it.id != action.userId }
                )
            }
            is AdminManagementAction.CreatePost -> {
                val newPost = com.androidfinalproject.hacktok.model.Post(
                    id = org.bson.types.ObjectId(),
                    content = action.content,
                    user = state.users.firstOrNull() ?: User(
            ...
            is AdminManagementAction.DeletePost -> {
                state = state.copy(
                    posts = state.posts.filter { it.id != action.postId }
                )
            }
            is AdminManagementAction.EditComment -> {
                state = state.copy(
                    comments = state.comments.map { comment ->
                        if (comment.id == action.commentId) comment.copy(comment = action.newContent) else comment
                    },
            ...
            is AdminManagementAction.DeleteComment -> {
                state = state.copy(
                    comments = state.comments.filter { it.id != action.commentId }
                )
            }
